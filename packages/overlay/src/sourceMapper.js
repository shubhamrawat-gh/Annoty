...
function getCssSelector(el) {
    ...
    while (current && current.nodeType === Node.ELEMENT_NODE) {
        ...
        if (parentElement) {
            ...
            const siblings = children.filter((c) => c.nodeName === current.nodeName);
 ...
 */
export function mapElementToSource(el) {
    ...
    try {
        const vueKeys = Object.keys(el).filter((key) => key.startsWith('__vue') || key === '__vueParentComponent');
    ...
    // --- Tier 5: Svelte Walk ---
    try {
        const svelteMeta = el.__svelte_meta;
    ...
    try {
        ...
        if (astroFile) {
            ...
            return {
                ...
                lineNumber: isNaN(line) ? undefined : line,
                columnNumber: isNaN(col) ? undefined : col,
            };
        }
    }
    catch (err) {
    ...
    // --- Tier 7: Preact Walk ---
    try {
        const vnodeKey = Object.keys(el).find((key) => key.startsWith('__vnode') || key.startsWith('__preactattr_'));
        if (vnodeKey) {
    ...
    try {
        ...
        if (srcFile) {
            ...
            return {
                ...
                lineNumber: isNaN(lineNum) ? undefined : lineNum,
                columnNumber: isNaN(colNum) ? undefined : colNum,
            };
        }
    }
    catch (err) {
