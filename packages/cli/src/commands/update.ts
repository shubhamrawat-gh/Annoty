...
export async function updateCommand() {
  ...
  try {
    ...
    const response = await fetch('https://registry.npmjs.org/annoty-cli/latest');
    if (!response.ok) {
    ...
    if (!confirmUpdate.proceed) {
      ...
      console.log(`To run the latest version on the fly, use: ${pc.bold('npx annoty-cli@latest <command>')}\n`);
      return;
    }

    console.log(pc.cyan('\nUpdating annoty-cli to the latest version...'));
    execSync('npm install -g annoty-cli@latest', { stdio: 'inherit' });
    console.log(pc.bold(pc.green('\n✓ Update complete! Please check the version using: annoty --version\n')));

  } catch (err: any) {
    console.log(pc.red(`\n✗ Error checking for updates: ${err.message}`));
    console.log(`You can update manually using: ${pc.bold('npm install -g annoty-cli@latest')}\n`);
  }
}
